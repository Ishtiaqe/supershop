...
- For Cloud SQL integration pass a substitution containing the full Cloud SQL instance string, e.g.: `--substitutions=_CLOUDSQL_INSTANCE=PROJECT:REGION:INSTANCE`. This value is used by the `--add-cloudsql-instances` flag and the job creation step.
