...
export default function DataManagementPage() {
  ...
  const handleExportBackup = async () => {
    if (!user) { message.error("Please log in to export backup"); return; }
    try {
  ...
  const handleImportBackup = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!user) { message.error("Please log in to import backup"); return; }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <Alert
        ...
        description={
          <div>
            ...
            <p className="mt-2">
              See our{" "}
              <Link to="/docs/data-safety" className="text-blue-500 underline">
          ...
          <Space direction="vertical" size="large" style={{ width: "100%" }}>
            <div>
              ...
              {backupStatus.data.backupSize && (
                <p><strong>Size:</strong> {formatBytes(backupStatus.data.backupSize)}</p>
              )}
            </div>
          </Space>
        </Card>
      ...
      <Card className="mb-6">
        ...
        <p className="text-gray-600 mb-4">
          <strong>⚠️ Warning:</strong> Restoring will replace all current data. Always backup first!
        </p>
        ...
        <Space>
          ...
          <Button
            ...
            disabled={importing}
          >
            {importing ? `Restoring... ${Math.round(uploadProgress)}%` : "Upload & Restore Backup"}
          </Button>
        </Space>
      </Card>
